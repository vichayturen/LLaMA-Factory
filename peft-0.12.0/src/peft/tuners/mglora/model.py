...
from .layer import MgLoraLayer, RankAllocator, SVDLinear


class MgLoraModel(LoraModel):
    """
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        if not isinstance(target, MgLoraLayer):
    ...
    def forward(self, *args, **kwargs):
        ...
        if (getattr(outputs, "loss", None) is not None) and isinstance(outputs.loss, torch.Tensor):
            ...
            for n, p in self.model.named_parameters():
                if ("lora_A" in n or "lora_B" in n) and self.trainable_adapter_name in n:
                    ...
                    if p.shape == torch.Size([0]):
                        ...
                        para_cov = p @ p.T if "lora_A" in n else p.T @ p
                    I = torch.eye(*para_cov.size(), out=torch.empty_like(para_cov)).to(para_cov.device)  # noqa: E741
                    I.requires_grad = False
                    num_param += 1
                    regu_loss += torch.norm(para_cov - I, p="fro").to("cpu")
            if num_param > 0:
            ...
            outputs.loss = outputs.loss * (1 + 2 * torch.sigmoid(regu_loss))
        return outputs
